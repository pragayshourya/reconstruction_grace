...
class DataFormatter:
    ...
    def convert_dataframe(self, return_data=True):
        ...
        x_pred = np.array([x[i] for i in np.where(np.isnan(y.reshape(-1)))]).squeeze() #x_pred is the missing months value in the GRACE-TWS time series.
        x_pred = x_pred[np.where(~np.isnan(y))[0][0]:] # The prepared datasets starts 1 year before the GRACE TWS datasets to incorpate lag value for the first observation also.

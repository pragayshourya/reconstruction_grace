...
if os.path.exists('./mkdir') is not True:
    os.makedirs('./mkdir')
...
#     print(file)
    ...
    date = []
    temp = pd.DataFrame(list(zip(act.reshape(-1,), pred.reshape(-1,))), columns=['GRACE-Actual', 'GRACE-Predicted'])
    for i in range(len(temp)):
        p = temp.iloc[i]
        for j in range(len(df)):
            m = df.iloc[j]
            if m[-1] == p[0]:
                date.append(df.index[j])
    ...
    recon_grace = recon_grace.merge(temp, how='left', left_index=True, right_index=True)
    for i in y_pred:
        for n,j in enumerate(recon_grace['GRACE-Predicted']):
            if isnan(j):
                recon_grace['GRACE-Predicted'][n] = i
                break
...
if __name__ == '__main__':
    ...
    pool.map(training_cnn_lstm, tqdm(files,bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}'))
    os.system('clear')
